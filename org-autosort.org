...
:PROPERTIES:
:SORT:     (org-todo-state-custom-down)
:END:
...
:END:
*** TODO get rid of annoying unfolding after =org-sort=
:PROPERTIES:
...
:END:
*** TODO put buffer name in error report for wrong element of sorting strategy
:PROPERTIES:
...
:END:
*** TODO do not raise error but put a message and do not sort on wrong :SORTING: format
:PROPERTIES:
